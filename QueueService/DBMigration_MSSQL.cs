...
namespace Tobasa
{
    internal class DBMigration_MSSQL
    {
        ...
        public static string v_queue_posts_summary =
            ...
            ,( SELECT station       FROM queue_sequences WHERE status = 'PROCESS' AND post = p.name AND date = CAST(getdate() as DATE)
                  AND number = ( SELECT MAX(number) FROM queue_sequences WHERE status = 'PROCESS' AND post = p.name AND date = CAST(getdate() as DATE ))
             ) AS last_station
            FROM queue_posts p;
            ";
        ...
        public static string v_queue_posts_summary_a =
            ...
            per_post_stats AS (
              ...
              FROM today_sequences
              GROUP BY post
            ),
            last_station_info AS (
              SELECT ts.post, ts.station
              FROM today_sequences ts
              INNER JOIN (
                SELECT post, MAX(number) AS max_number
                FROM today_sequences
                WHERE status = 'PROCESS'
                GROUP BY post
              ) AS latest
              ON ts.post = latest.post AND ts.number = latest.max_number
              WHERE ts.status = 'PROCESS'
            )
        ...
        public static string cmd_insert_all_basic_data =
            ...
            INSERT INTO #temp_posts (name, keterangan, numberprefix) VALUES
               ( 'POST0', 'Poli Gigi', 'A'),
               ( 'POST1', 'Poli Mata', 'B'),
               ( 'POST2', 'Poli Kandungan', 'C'),
               ( 'POST3', 'Poli Bedah', 'D'),
               ( 'POST4', 'Poli Anak', 'E'),
               ( 'POST5', 'Poli Ortopedi', 'F'),
               ( 'POST6', 'Poli Jantung', 'G'),
               ( 'POST7', 'Poli THT', 'H'),
               ( 'POST8', 'Poli Kulit', 'I'),
               ( 'POST9', 'Poli Dalam', 'J'),
               ( 'POST10', 'Poli Saraf', 'K'),
               ( 'POST11', 'Poli Psikiatri', 'L'),
               ( 'POST12', 'Poli Paru', 'M'),
               ( 'POST13', 'Rehab Medik', 'N');
